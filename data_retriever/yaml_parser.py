...
def load_plan_from_yaml(file_path: str) -> tuple[VCenter, UpsGrace, Servers]:
    ...
    Returns:
        tuple[VCenter, UpsGrace, Servers]: A `VCenter` object, an `UpsGrace` object and a list of `Server` objects representing the migration plan, or `None` if an error occurs
     """
    try:
